...
> If the data is linearly separable, the algorithm will converge after $<\frac{R^2}{\gamma^2}$ mistakes where $R = \max_i||x_i||_2$ (size of biggest $x_i$) and margin $\gamma < y_iw^Tx_i$.
...
For simple perceptrons, $\eta = 1$; with passive-aggressive perceptrons, $$\eta = L(w | x_i, y_i) / ||x_i||^2$$where $L$ is the hinge loss $L(w | x_i, y_i) = \max(0, 1 - y_iw^Tx_i)$.
